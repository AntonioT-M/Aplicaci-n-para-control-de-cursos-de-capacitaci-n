...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 02-07-2020 a las 21:30:08
...
-- Procedimientos
--
CREATE  PROCEDURE `sp_addalu`(IN `cur` VARCHAR(19), IN `telal_idtel` INT(11), IN `nomalu` VARCHAR(50), IN `ap1` VARCHAR(30), IN `ap2` VARCHAR(30), IN `cor` VARCHAR(100), IN `cpa` INT(5))
BEGIN
...
CREATE  PROCEDURE `sp_addaula`(IN idau INT(11), IN nomau VARCHAR(50), IN est VARCHAR(11))
BEGIN
...
CREATE  PROCEDURE `sp_addaut`(IN `id` INT(10), IN `sol` INT(10), IN `idpag` INT(11), IN `mont` DOUBLE(9,2))
begin
...
CREATE  PROCEDURE `sp_addbec`(IN `idbec` INT(10), IN `nombec` VARCHAR(25), IN `mon` INT(100))
BEGIN
...
CREATE  PROCEDURE `sp_addcur`(IN idcur INT(10), IN idins INT (11), IN nomcur VARCHAR(100), IN imgcur VARCHAR(200), 
...
CREATE  PROCEDURE `sp_addesp`(IN idesp INT(11), IN esp VARCHAR(50), IN des varchar(150))
BEGIN
...
CREATE  PROCEDURE `sp_addgpo`(IN idgpo INT(10), IN idcur INT(11),IN idaul INT(11), IN idhor INT(11), 
...
CREATE  PROCEDURE `sp_addhor`(IN idhor int (10), IN inihor time, IN terhor time, IN diahor varchar(20))
BEGIN
...
CREATE  PROCEDURE `sp_addins`(IN `idins` INT(11), IN `idesp` INT(11), IN `idtip` INT(11), IN `idtel` INT(11), IN `nomins` VARCHAR(50), IN `ap1` VARCHAR(30), IN `ap2` VARCHAR(30))
BEGIN
...
CREATE  PROCEDURE `sp_addinsc`(IN `id` INT(11), IN `gpo` INT(10), IN `alu` VARCHAR(18), IN `pag` INT(11))
BEGIN
...
CREATE  PROCEDURE `sp_addpago`(IN `idpag` INT(10), IN `idcur` INT(10), IN `pag` DOUBLE(9,2), IN `fecp` DATE, IN `est` VARCHAR(15))
BEGIN
     declare curexiste int;
     declare existe int;
     declare mon double(9,2);
...
CREATE  PROCEDURE `sp_addsol`(solid int(10), alusol varchar(19), solbec int(10), solest varchar(30))
begin
declare existe int;
declare existe2 int;
declare existe3 int;
...
CREATE  PROCEDURE `sp_addtel`(IN idtel INT(11), IN tptel INT(11), IN tel varchar(20))
BEGIN
...
CREATE  PROCEDURE `sp_addtip`(IN idtip INT(11), IN tip VARCHAR(12), IN des varchar(150))
BEGIN
...
CREATE  PROCEDURE `sp_addtipotel`(IN idtptel INT(11), IN tptel VARCHAR(50))
BEGIN
	 declare tpexiste int;
...
CREATE  PROCEDURE `sp_busalu`(IN `buscur` VARCHAR(19))
BEGIN
	declare aluexiste int;
...
CREATE  PROCEDURE `sp_busaul`(IN `idau` INT(11))
BEGIN
	declare idexiste int;
...
CREATE  PROCEDURE `sp_busaut`(IN `autid` INT(10))
begin
...
CREATE  PROCEDURE `sp_busbec`(IN `idbec` INT(10))
BEGIN
	declare aluexiste int;
			if idbec = 0 then
			select concat("Ingresa el id de la beca") as mensaje;
...
CREATE  PROCEDURE `sp_buscur`(IN `idcur` INT(11))
BEGIN
	declare idexiste int;
...
CREATE  PROCEDURE `sp_busesp`(IN `idesp` INT(11))
BEGIN
	declare idexiste int;
...
CREATE  PROCEDURE `sp_busgru`(IN `idgru` INT(11))
BEGIN
	declare aluexiste int;
			if idgru = 0 then
...
CREATE  PROCEDURE `sp_bushor`(IN `idhor` INT(10))
BEGIN
	declare aluexiste int;
			if idhor = 0 then
...
CREATE  PROCEDURE `sp_busins`(IN `busins` INT(11))
BEGIN
...
CREATE  PROCEDURE `sp_businst`(IN `businst` INT(10))
BEGIN
	declare aluexiste int;
...
CREATE  PROCEDURE `sp_buspago`(IN `idpag` INT(10))
BEGIN
	declare pagexiste int;
			if idpag = 0 then
			select concat("Ingresa el id de la beca") as mensaje;
...
CREATE  PROCEDURE `sp_bussol`(IN `solid` INT(10))
begin
declare existe int;
...
CREATE  PROCEDURE `sp_bustel`(IN `id` INT(11))
BEGIN
	declare idexiste int;
...
CREATE  PROCEDURE `sp_bustels`(IN id INT)
BEGIN
select idTel, tipoTel_idTipTel, telefono from telefono where idTel = id;
END$$

CREATE  PROCEDURE `sp_bustip`(IN `idtip` INT(11))
BEGIN
...
CREATE  PROCEDURE `sp_bustptel`(IN idtptel int(11))
BEGIN
	declare idexiste int;
...
CREATE  PROCEDURE `sp_delaut`(IN id INT(11))
begin
...
CREATE  PROCEDURE `sp_delhorr`(iddel int (11))
begin
...
CREATE  PROCEDURE `sp_delins`(IN ins INT(11), IN id INT(11))
begin
...
CREATE  PROCEDURE `sp_delpag`(IN id INT(11))
begin
...
CREATE  PROCEDURE `sp_enlal`()
begin
	select * from alumnos;

end$$

CREATE  PROCEDURE `sp_enlaul`()
begin
	select * from aulas;

end$$

CREATE  PROCEDURE `sp_enlbec`()
begin
	select * from becas;

end$$

CREATE  PROCEDURE `sp_enlcur`()
begin
	select * from cursos;

end$$

CREATE  PROCEDURE `sp_enlesp`()
begin
	select * from especialidad;

end$$

CREATE  PROCEDURE `sp_enlgpo`()
begin
	SELECT * FROM grupo JOIN cursos ON grupo.cursos_idCursos = cursos.idCursos;

end$$

CREATE  PROCEDURE `sp_enlhor`()
begin
	select * from horario;

end$$

CREATE  PROCEDURE `sp_enlins`()
begin
...
CREATE  PROCEDURE `sp_enlpag`()
begin
	select idPago, nombre_curso, imgcurso, pago, fechaP, estatusP from pagos JOIN cursos ON pagos.id_cur = cursos.idCursos;

end$$

CREATE  PROCEDURE `sp_enltel`()
begin
	select * from telefono;

end$$

CREATE  PROCEDURE `sp_enltip`()
begin
	select * from tipo;

end$$

CREATE  PROCEDURE `sp_enltptel`()
    NO SQL
begin
	SELECT * FROM tipotel;
end$$

CREATE  PROCEDURE `sp_lisaut`()
begin
...
CREATE  PROCEDURE `sp_updalu`(IN cp VARCHAR(19), IN tel INT(11), IN nmalu VARCHAR(50), IN ap1 VARCHAR(30), IN ap2 VARCHAR(30),
...
CREATE  PROCEDURE `sp_updaul`(IN `id` INT(11), IN `nom` VARCHAR(50), IN `est` VARCHAR(13))
BEGIN
	declare existe int;
...
CREATE  PROCEDURE `sp_updaut`(autid int(10), solaut int(10),IN idpago INT(11), autmon double(9,2))
begin
declare existe int;
declare solexiste int;
declare pagexiste int;
...
CREATE  PROCEDURE `sp_updbeca`(idbec int(10), nombec varchar(25), desbec int(5))
begin
declare existe int;
...
CREATE  PROCEDURE `sp_updcur`(IN idcur INT(10), IN ins_idins_cur INT (11), IN nomcur VARCHAR(100), 
...
CREATE  PROCEDURE `sp_updesp`(IN id INT(11), IN esp VARCHAR(50), IN des VARCHAR(150))
BEGIN
	declare existe int;
...
CREATE  PROCEDURE `sp_updgpo`(IN idgpo INT(10), IN idcur INT(11), IN idaul INT(11), 
...
CREATE  PROCEDURE `sp_updhor`(IN idhor int (10), IN inihor time, IN terhor time, IN diahor varchar(20))
BEGIN
	declare existe int;
    select trim(diahor) into diahor;
    set diahor= trim(diahor);
...
CREATE  PROCEDURE `sp_updins`(IN idins int(11),IN idesp INT(11),IN idtip INT(11), IN idtel INT(11), IN nomins VARCHAR(50),
...
CREATE  PROCEDURE `sp_updinsc`(IN `id` INT, IN `gpo` INT, IN `alu` VARCHAR(18), IN `pag` INT)
    NO SQL
BEGIN
...
CREATE  PROCEDURE `sp_updinst`(IN `id` INT(11), IN `gpo` INT(10), IN `alu` VARCHAR(18), IN `pag` INT(11))
BEGIN
	DECLARE existe int;
    DECLARE gpoexiste int;
    DECLARE aluexiste int;
...
CREATE  PROCEDURE `sp_updpago`(IN idpag INT(10), IN idcur int(10), IN pag DOUBLE(9,2),
...
CREATE  PROCEDURE `sp_updsol`(IN `solid` INT(10), IN `alusol` VARCHAR(19), IN `solbec` INT(10), IN `solest` VARCHAR(30))
begin
declare existe int;
declare existe2 int;
declare existe3 int;
...
CREATE  PROCEDURE `sp_updtel`(IN id INT(11), IN idtip INT(11), IN tel VARCHAR(20))
BEGIN
	declare existe int;
    declare tipexiste int;
...
CREATE  PROCEDURE `sp_updtip`(IN id INT(11), IN tp VARCHAR(12), IN des VARCHAR(150))
BEGIN
	declare existe int;
...
CREATE  PROCEDURE `sp_updtptel`(IN id INT(11), IN tip VARCHAR(50))
BEGIN
	declare existe int;
...
(1312, 2, 'Bufandas y Gorros', 'uploads/Bufandas160420102454.jpg', '2020-04-20', '2020-04-09', 'Aprende a cocer bufandas y gorros de manera profesional, obteniendo de esta manera un titulo con el cual desempeÃ±arte, ademas de tener la posibilidad de crear tu propio negocio', 200.00);
